...
const CountryAdminInfoCard: React.FC<ConsultantInfoCardProps> = ({ countryAdmin }) => {
      ...
      <div className="flex flex-col flex-1 justify-between items-center border-r px-10 border-r-verido-border">
        <div className="flex items-center justify-center flex-col gap-2">
          <Image
            ...
            width={30}
            height={30}
            alt={countryAdmin.email}
            className="w-16 h-16 rounded-full object-contain"
          />
          <div className="flex flex-col justify-center items-center gap-1">
            <h2 className="text-lg">{countryAdmin.username}</h2>
            <p className="text-gray-text text-[10px] ">{countryAdmin.email}</p>
            <Button
              size={"sm"}
      ...
      <div className="flex flex-col items-start justify-start gap-3 w-full ">
        <div className="flex justify-between items-start w-full">
          <div className="flex flex-col flex-1 gap-2 ">
            <h3 className="text-2xl">Country Admin&apos;s Informations</h3>
        ...
        <div className="border-t border-verido-border w-full py-4">
          ...
          <p className="text-gray-text text-[12px]">{countryAdmin.email}</p>
        </div>
        <div className="border-t border-verido-border py-4 flex flex-col gap-5 w-full">
          ...
          <div className="flex gap-10">
            ...
            <ul className="flex flex-col gap-4 text-[12px] text-gray-text">
              <li>{countryAdmin.email}</li>
              <li>{countryAdmin.institution}</li>
              <li>{countryAdmin.email}</li>
              <li>{countryAdmin.email}</li>
              <li>{countryAdmin.email}</li>
              <li>{countryAdmin.email}</li>
        ...
        <div className="border-t border-verido-border flex flex-col gap-5 py-4 w-full">
          ...
          <div className="flex gap-24">
            ...
            <ul className="flex flex-col gap-4 text-[12px] text-gray-text">
              <li>{countryAdmin.email}</li>
              <li className="flex">{renderStars(countryAdmin.rating)}</li>
            </ul>
          </div>
        </div>
      </div>
