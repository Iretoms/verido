...
export interface ICreateSuperAgent {
  email: string;
  password: string;
  phoneNumber: string;
  name: string;
...
const SuperAgentForm = ({ closeModal }: SuperAgentProps) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const { createSuperAgentMutation } = useUser();

  const onSubmit = async (data: ICreateSuperAgent) => {
    if (isSubmitting) return;
    try {
      await createSuperAgentMutation.mutateAsync(data);
      closeModal();
      reset();
    } catch (error) {
      console.error(error);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex flex-col gap-1 items-start">
          ...
          <Input
            ...
            id="partnerId"
            {...register("partnerId", {
              required: "Your partnerId is required",
            })}
            className={`border ${
          ...
          />
          {errors.partnerId && (
            <p className="text-red-500 text-xs">{errors.partnerId.message}</p>
          )}
        </div>
        <div className="flex flex-col gap-1 items-start">
